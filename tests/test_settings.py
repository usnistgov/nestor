...
small_dict = {"I": {"A": 1}, "II": "B", "III": {"C": {"2": "a"}, "D": {"3": "b"},}}

nestorParams = ns.nestor_params()


@pytest.mark.parametrize(
    "test_path,retrieved", [("I.A", 1), ("III.C.2", "a"), ("III.D", {"3": "b"})]
)
...
@pytest.mark.parametrize(
    "test_val,found_path", [(1, "I.A"), ("a", "III.C.2"), ("3", "III.D.3")]
)
...
def test_default_entities():
    assert set(nestorParams.entities) == {"P", "I", "S", "PI", "SI", "U", "X"}
...
def test_default_derived():
    assert set(nestorParams.derived) == {"PI", "SI"}


@pytest.mark.parametrize(
    "combine,result", [("P I", "PI"), ("I P", "PI"), ("I I", "I"), ("P P", "X")]
)
...
def test_default_datatype():
    ...
    assert ("technician.name" in names) and (names[0] in nestorParams.datatypes)
