...
def tag_relation_net(
    ...
    name=None,
    kind='coocc',
    layout=<function fruchterman_reingold_layout at 0x7f3e325f1820>,
    layout_kws=None,
    padding=None,
    **node_adj_kws
)
...
|---|---|---|---|
| tag_df | pandas.DataFrame | standard Nestor tag occurrence matrix. Multi-column with top-level containing
tag classifications (named-entity NE) and 2nd level containing tags. Each row
corresponds to a single event (MWO), with binary indicators (1-occurs, 0-does not). | None |
| name | str | what to name this tag relation element. Creates a Holoviews group "name". | None |
| kind | str | coocc :
    co-occurrence graph, where tags are connected if they occur in the same MWO,
    above the value calculated for pct_thres. Connects all types together.
sankey :
    Directed "flow" graph, currently implemented with a (P) -> (I) -> (S) structure.
    Will require ``dag=True``. Alters default to ``similarity=count`` | to |
| layout | object (function) | must take a graph object as input and output 2D coordinates for node locations
(e.g. all networkx.layout functions). Defaults to ``networkx.spring_layout`` | s |
| layout_kws | dict | options to pass to networkx layout functions | None |
| padding | dict | contains "x" and "y" specifications for boundaries. Defaults:
    ``{'x':(-0.05, 1.05), 'y':(-0.05, 1.05)}``
Only valid if ``kind`` is 'coocc'. | s |
| node_adj_kws |  | keyword arguments for ``nestor.tagtrees.tag_df_network``. Valid options are
    similarity : 'cosine' (default) or 'count'
    dag : bool, default='False', (True if ``kind='sankey'``)
    pct_thres : : int or None
        If int, between [0,100]. The lower percentile at which to
        threshold edges/adjacency. | None |

**Returns:**

| Type | Description |
|---|---|
| holoviews.Holomap or holoviews.Graph element, pending sankey or cooccurrence input. | None |
...
tag classifications (named-entity NE) and 2nd level containing tags. Each row
corresponds to a single event (MWO), with binary indicators (1-occurs, 0-does not). | None |
| how | string | Method for resampling data by day. If `None`, assume data is already
sampled by day and don't resample. Otherwise, this is passed to Pandas
`Series.resample`. | None |
| yearlabels | bool | Whether or not to draw the year for each subplot. | None |
| yearascending | bool | Sort the calendar in ascending or descending order. | None |
| yearlabel_kws | dict | Keyword arguments passed to the matplotlib `set_ylabel` call which is
used to draw the year for each subplot. | None |
| subplot_kws | dict | Keyword arguments passed to the matplotlib `add_subplot` call used to
create each subplot. | None |
| gridspec_kws | dict | Keyword arguments passed to the matplotlib `GridSpec` constructor used
to create the grid the subplots are placed on. | None |
| fig_kws | dict | Keyword arguments passed to the matplotlib `figure` call. | None |
| kwargs | other keyword arguments | All other keyword arguments are passed to `yearplot`. | None |

**Returns:**

| Type | Description |
|---|---|
| matplotlib Figure and Axes | Tuple where `fig` is the matplotlib Figure object `axes` is an array
of matplotlib Axes objects with the calendar heatmaps, one per year. |
...
tag classifications (named-entity NE) and 2nd level containing tags. Each row
corresponds to a single event (MWO), with binary indicators (1-occurs, 0-does not). | None |
| year | integer | Only data indexed by this year will be plotted. If `None`, the first
year for which there is data will be plotted. | None |
| how | string | Method for resampling data by day. If `None`, assume data is already
sampled by day and don't resample. Otherwise, this is passed to Pandas
`Series.resample`. | None |
| vmin, vmax | floats | Values to anchor the colormap. If `None`, min and max are used after
resampling data by day. | None |
| cmap | matplotlib colormap name or object | The mapping from data values to color space. | None |
| linewidth | float | Width of the lines that will divide each day. | None |
| linecolor | color | Color of the lines that will divide each day. If `None`, the axes
background color is used, or 'white' if it is transparent. | None |
| monthlabels | list | Strings to use as labels for months, must be of length 12. | None |
| monthticks | list or int or bool | If `True`, label all months. If `False`, don't label months. If a
list, only label months with these indices. If an integer, label every
n month. | None |
| ax | matplotlib Axes | Axes in which to draw the plot, otherwise use the currently-active
Axes. | None |
| kwargs | other keyword arguments | All other keyword arguments are passed to matplotlib `ax.pcolormesh`. | None |

**Returns:**

| Type | Description |
|---|---|
| matplotlib Axes | Axes object with the calendar heatmap. |
...
| Name | Type | Description | Default |
|---|---|---|---|
| obj_type | passed to filter_type_name | None | None |
| obj_name | passed to filter_type_name | None | None |
| n_thres | only return nodes in the top ``n_thres`` percentile | None | None |

**Returns:**

| Type | Description |
|---|---|
| pd.DataFrame, filtered binary tax matrix | None |
...
| Name | Type | Description | Default |
|---|---|---|---|
| obj_type | class of object to filter on | None | None |
| obj_name | sub-class/instance to filter on | None | None |

**Returns:**

| Type | Description |
|---|---|
| pd.Series, mask for filtering df, tag_df. | None |
...
| Name | Type | Description | Default |
|---|---|---|---|
| obj_type | class of object to show | None | None |
