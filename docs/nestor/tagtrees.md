...
|---|---|---|---|
| df | a dataframe containing columns of tag assignments (comma-sep, str) | None | None |
| col | which column to extract | None | None |
| topn | how many of the top most frequent tags to return | None | None |

**Returns:**

| Type | Description |
|---|---|
| list of (tag,count,numpy.array) tuples | None |
...
| Name | Type | Description | Default |
|---|---|---|---|
| dist_mat | pandas.DataFrame | contains similarity matrix, indexed and named by tags | None |
...
btw: betweenness
cls: closeness | None |
| tau | float | similarity threshold for retaining a node | None |
| dynamic | bool | whether to re-calculate centrality after popping every tag | None |
| write_dot | str or None | file location, where to save a .dot, if any. | None |
| verbose | bool | print some stuff | None |
...
| similarity | str | cosine: cosine similarity (from ``sklearn.metrix.pairwise``)
count: count (the number of co-occurrences of each tag-tag pair) | None |
| dag | bool | default adj_mat will be accross all nodes. This option will return a
directed, acyclic graph (DAG), useful for things like Sankey Diagrams.
Current implementation returns (P) -> (I) -> (S) structure (deletes others). | adj_mat |
| pct_thres | int or None | If int, between [0,100]. The lower percentile at which to threshold edges/adjacency. | None |

**Returns:**

| Type | Description |
|---|---|
| pandas.DataFrame, containing adjacency measures for each tag-tag (row-column) occurrence | None |
